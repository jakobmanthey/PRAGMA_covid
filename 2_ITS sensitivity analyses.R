...
acf(res) # has autocorrelation and also indications of seasonality
pacf(res) # also seasonality
...
acf(res) # looks mostly ok
pacf(res) # perhaps MA1
tseries::adf.test(res) # stationarity: -6; p<.01 = ok

##  identity autocorrelation (AR/MA terms) using AUTO.ARIMA
auto.arima(resid(pri.mod.1$lme, type = "normalized"),
           stationary = TRUE, seasonal = FALSE) # AR2  -> but use MA2 according to ACF/pACF plot
...
auto.arima(resid(sec1.mod.1$lme, type = "normalized"),
           stationary = TRUE, seasonal = FALSE) # AR4 and MA4 but ok according to ACF/pACF --> no GAMM
...
summary(-(1-obs/alt)) # median: -55.9%
rm(dat.sec2, obs, alt)

##  lockdown 2:
dat.sec2 <- copy(data[,.(period,secondary_inp,time,lockdown1,time_since_lockdown1,
                         time_since_lockdown1,time_since_between_lockdowns,
                         lockdown2 = 0,time_since_lockdown2 = 0,time_since_after_lockdowns,
                         week)])

dat.sec2$pred <- predict(sec2.mod.2$gam)
dat.sec2$pred_counter <- predict(sec2.mod.2$gam, newdata = dat.sec2)
dat.sec2[period == 4,.(time,secondary_inp,pred,pred_counter)]
dat.sec2[period == 4,.(time,secondary_inp)][,summary(secondary_inp)]
dat.sec2[period == 4,.(time,pred_counter)][,summary(pred_counter)]

obs <- dat.sec2[period == 4]$secondary_inp
alt <- dat.sec2[period == 4]$pred_counter

summary(-(1-obs/alt)) # median: -11.9%
rm(dat.sec2, obs, alt)
...
sjPlot::tab_model(pri.mod.2,
                  ...
                  file = paste0("tables/Supp Table2_model summary_sensitivity_",Sys.Date(),".html"))


# 7) EXPORT
# ______________________________________________________________________________________________________________________

saveRDS(pri.mod.2, file = paste0("data/output/",DATE,"_primary outcome_model_sensitivity.rds"))
saveRDS(sec1.mod.1, file = paste0("data/output/",DATE,"_secondary outcome1_model_sensitivity.rds"))
